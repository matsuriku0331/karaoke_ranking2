...
# Render/Heroku等のレシーバによってはsslmodeが必要な場合あり
...
# 3人限定: 表示名 -> Cookie情報
# ★本番では環境変数で注入することを推奨（例: USER1_CDM, USER1_UID, ...）
USER_COOKIES = {
    "まつりく": {
        "dam-uid": os.environ.get("U1_DAM_UID", "5c6851a696db3f0488cea63754f839d70810012f131432c88a487c2227af8429"),
        "scr_cdm": os.environ.get("U1_SCR_CDM", "ODAwMDEwMjY3MjIwOTY5"),
        "scr_dt":  os.environ.get("U1_SCR_DT",  "MTI0NTUwMDU0"),
        "webmember": "1",
        "wm_ac": os.environ.get("U1_WM_AC", "matsuriku0331"),
        "wm_dm": os.environ.get("U1_WM_DM", "softbank.ne.jp")
    },
    # 友だちA/Bは後で本番Cookieを設定
    "ともだちA": {
...
# 3人限定のシンプル認証（ユーザー名の選択 + 合言葉）
# 本番では最低限 .env/環境変数で管理してください
USER_PASSWORDS = {
    "まつりく": os.environ.get("U1_PASS", "pass1"),
    "ともだちA": os.environ.get("U2_PASS", "pass2"),
    "ともだちB": os.environ.get("U3_PASS", "pass3"),
}

# ---- Helper: Japanese sort key (locale優先/フォールバック) ----
...
def login():
    if request.method == "POST":
        ...
        if username in USER_PASSWORDS and password == USER_PASSWORDS[username]:
            session['login_user'] = username
            flash(f"{username} としてログインしました", 'success')
            return redirect(url_for('ranking'))
        else:
            flash("ユーザー名または合言葉が違います", 'danger')
    return render_template("login.html", users=list(USER_PASSWORDS.keys()))

@app.route("/logout")
@login_required
def logout():
    u = session.pop('login_user', None)
